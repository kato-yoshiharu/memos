...
オンラインエディタの開発では、テーブルの複数セルのセレクションや行・列の削除、マージされたセルを考慮したリサイズ、インデントなど、状態と座標計算が複雑に絡む機能を数多く実装してきました。
Popoverの座標計算を画面の枠からはみ出るケースまで考慮して作り直すなど、ライブラリに頼らず要件に合わせてゼロから設計・実装できることが強みです。
