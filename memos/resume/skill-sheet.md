...
技術スタック: TypeScript/Next.js/Tailwind CSS/shadcn + Radix UI/assistant-ui/Orval/Storybook/TanStack Query/nuqs/Vitest/Testing Library/Playwright
規模: PdM1名 / バックエンド1名 / フロントエンド1名
担当工程: 要件定義 / 基本設計 / 詳細設計 / 実装 / テスト

#### 案件概要

クライアントの自社製品に関する問い合わせの回答をサポートするチャットボットの受託開発。
フロントエンドの開発を立ち上げから1人で担当し、PdMと連携してプロダクトロードマップの策定や要件定義・仕様策定にも参画。
クライアント企業の担当者と意見交換を重ね、合意形成を行いながら開発を推進。
限られた時間の中で、要件の変化に応じてPdMとタスクの優先順位を都度見直しながら開発を進めた。
技術選定・設計・開発フロー・実装方針などの意思決定をドキュメントとして明文化し、後から参画するメンバーの手助けとなる基盤を整えた。
仕様書をもとにしたTDDで実装前にテストケースを整理することで、仕様の抜け漏れを防ぎつつ変更に強いコードを維持。
